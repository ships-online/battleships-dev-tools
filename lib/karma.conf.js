...
module.exports = ( config, options = {} ) => {
	...
	const karmaConfig = {
		...
		preprocessors: {
			'tests/**/*.js': [ 'webpack' ],
			'packages/battleships-core/tests/**/*.js': [ 'webpack' ],
			'packages/battleships-engine/tests/**/*.js': [ 'webpack' ],
			'packages/battleships-ui-vanilla/tests/**/*.js': [ 'webpack' ]
		},
	...
	if ( options.files ) {
		karmaConfig.files = options.files.split( ',' ).map( ( file ) => {
			if ( !file.endsWith( '*.js' ) ) {
				return path.join( file, '**', '*.js' );
			}

			return file;
		} );

		for ( const file of karmaConfig.files ) {
	...
	if ( options.sourcemap ) {
		for ( const preprocessor in karmaConfig.preprocessors ) {

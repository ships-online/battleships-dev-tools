...
module.exports = ( config ) => {
	...
	const tasks = {
		...
		/**
		 * Compiles source files from given package to specified format and destination.
		 *
		 * @param {String} name Package short name (without `battleships-` prefix).
		 * @param {Object} [options={}] Additional options.
		 * @param {'esnext'|'cjs'} [options.format='esnext'] Output format.
		...
		/**
		 * Compiles game source files and selected theme.
		 *
		...
		compileGame( outputOptions, additionalOptions = {} ) {
			return new Promise( () => {
				...
				const promises = outputOptions.themes.map( ( themeName ) => {
					return buildTheme( themeName, outputOptions.destination );
				} ).concat( [ bundleJS( webpackConfig ) ] );

				if ( additionalOptions.watch ) {
					outputOptions.themes.forEach( ( themeName ) => {
						const themePath = getDependencyPath( `battleships-theme-${ themeName }` );
						const taskName = `_buildTheme-${ themeName }`;

						gulp.task( taskName, () => buildTheme( themeName, outputOptions.destination ) );
